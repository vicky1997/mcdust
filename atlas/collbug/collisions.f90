...
module collisions
   ...
   subroutine bugflag(nsws, swarms, flag, loc)
      implicit none
      type(swarm), dimension(:), intent(in) :: swarms
      integer, intent(inout) :: flag
      integer, intent(in) :: loc, nsws
      if (size(swarms)<nsws) then
         open(29,file='collflag.dat',status='unknown',position='append')
         flag = flag + 1
         write(29,*) nsws-size(swarms), flag, loc
         write(*,*)'WARNING', nsws-size(swarms), 'particles lost at', loc, 'location in collisions subroutine for the',flag, 'time'
         close(29)
      endif
