...
module hdf5output
  ...
  contains
  subroutine hdf5_file_write (this, swrm, time, mode)
    ...
    if (mode .eq. 'create') then
      call hdf5_file_create(this, filename)                   ! creates file
      call hdf5_create_dataspace(this, swrm)                  ! creates dataspace and adds t = 0  data
      timeout = time/year
  ...
  subroutine hdf5_create_dataspace (this, swrm)
    ...
    chunkdims(rank) = 1
    call swarm_unit_conversion(swrm, tempswrm)                   ! convert data unitless
...
! subroutine to add the swarm object as a compound datatype to the HDF5 file
  ...
  subroutine hdf5_file_create(this, filename)
    ...
    call hdf5_write_attribute(this%id, 'version', '0.')
