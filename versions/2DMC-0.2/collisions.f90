...
module collisions
   ...
   use parameters, only: matdens, r0, dmmax, con1, con2, vfrag, vertsett, rdrift
   ...
   subroutine collision(nri,nrk,swarms,relvels, accelncol)
      ...
      !endif
      else if (swarms(nri)%mass/swarms(nrk)%mass .ge. 10) then
         call erosion(nri,nrk,swarms,accelncol)
         write(*,*) 'in erosion line'
      else
         call fragmentation(nri, swarms)
         write(*,*) 'in fragmentation'
      endif
