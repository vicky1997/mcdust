...
module timestep
   ...
   subroutine time_step(bin, ncolls, dtout, final_dtime)
      ...
      final_dtime = min(final_dtime, 10*size(bin(1,1)%p) * old_dt / real(maxval(ncolls(:,:)))) !we don't want more collisions than ~ nr of particles in a zone per timestep
      ...
      do i = 1, size(ncolls(:,:),dim=1)
        ...
        !final_dtime = min(final_dtime, size(bin(i,j)%p) * old_dt / real(max(1,ncolls(i,j)))) !we don't want more collisions than ~ nr of particles in a zone per timestep
          if (vertsett) final_dtime = min(final_dtime, 10*g%dz(i,j)/maxval(abs(bin(i,j)%p(:)%velz))) ! don't let particles jump over cells in z
          if (rdrift) final_dtime = min(final_dtime, 10*g%dr(i)/maxval(abs(bin(i,j)%p(:)%velr))) ! don't let particles jump over cells in r
        enddo
      enddo
      !$OMP END PARALLEL DO
