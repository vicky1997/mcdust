...
module collisions
   ...
   use constants,  only: pi, third, year, smallv, AU, Ggrav, mH2, Msun, AH2
   use discstruct, only: cs, omegaK, densg, vgas, alpha, sigmag
   use grid,       only: g
   use initproblem,only: m0, mswarm
   use parameters, only: matdens, r0, dmmax, con1, con2, vfrag, vertsett, rdrift, eta, erosion_switch
   ...
   subroutine mc_collisions(nr, ni, bin, dtime, realtime, kk)
      ...
      real                                            :: dtg ! local dust to gas ratio!
      integer                                         :: i, k, l
      integer, intent(out)                            :: kk ! collisions counter
      real                                            :: dustdens

      ! calculation of some values needed for calculations (optimization)
      ...
      Reynolds = sqrt(0.5 * pi) * alpha(g%rce(nr)) * cs(g%rce(nr)) / (omegaK(g%rce(nr)) * lmfp) ! Reynolds number
      v0 = sqrt(alpha(g%rce(nr))) * cs(g%rce(nr))                                 ! velocity of the largest eddy
      ...
      vn =  eta * omegaK(g%rce(nr)) * g%rce(nr)                                   ! maximum radial velocity from the pressure gradient
      ...
      do i = 1, nsws
         call stokes_nr_centr(nr, i, swarms, stokesnr, lmfp, gasdens)
         call vel_vs_centr(nr, ni, i, stokesnr, vs)
        ! if (rdrift) then
      ...
      do i = 1, nsws
         call rel_vels(nr, i, swarms, stokesnr, vr, vs, relvels, vn, Reynolds, veta, Vg2, tL, teta)
      ...
      do while (local_time < dtime)
         ...
         call stokes_nr_centr(nr, nri, swarms, stokesnr, lmfp, gasdens)
         call vel_vs_centr(nr, ni, nri, stokesnr, vs)
         !if (rdrift) then
            call vel_rd_centr(nr, nri, stokesnr, vr, vn, realtime)
         !endif
         call rel_vels(nr, nri, swarms, stokesnr, vr, vs, relvels, vn, Reynolds, veta, Vg2, tL, teta)
         relvels(:,nri) = relvels(nri,:)
         call col_accel(nri ,swarms, accelncol)
         call col_accel_r(nri ,swarms, accelncol)
         colri(:) = colri(:) - colrates(:,nri)
         call col_rates(nri, swarms, relvels, colrates, accelncol, g%vol(nr,ni))
   ...
   subroutine stokes_nr_centr(nr, i, swarms, stokesnr, lmfp, gasdens)
      implicit none
      type(swarm), dimension(:)                       :: swarms
      integer, intent(in)                             :: nr
      integer, intent(in)                             :: i
      real, dimension(:), allocatable                 :: stokesnr
      real, intent(in)                                :: lmfp, gasdens
      real                                            :: rad

      rad = con2 * swarms(i)%mass**third                       ! particle radius

      if (rad > 2.25 * lmfp) then ! Stokes regime
         stokesnr(i) = sqrt(2.*pi) * matdens * AH2 * rad**2. * omegaK(g%rce(nr)) / (9. * mH2 * cs(g%rce(nr)) )
      else                        ! Epstein regime
         stokesnr(i) = rad * matdens / (sqrt(8./pi) * cs(g%rce(nr))  * gasdens) * omegaK(g%rce(nr))
      endif
   ...
   subroutine rel_vels(nr, nl, swarms, stokesnr, vr, vs, relvels, vn, Reynolds, veta, Vg2, tL, teta)
      implicit none
      ...
      ! Brownian motions
      vB2(:) = 8.* mH2 * cs(g%rce(nr))**2 * (swarms(nl)%mass+swarms(:)%mass) / (pi*swarms(nl)%mass*swarms(:)%mass)

      ! turbulence
      do i = 1, size(swarms)
         if (stokesnr(i) > stokesnr(nl)) then
            gts = stokesnr(i) / omegaK(g%rce(nr))
            lts = stokesnr(nl) / omegaK(g%rce(nr))
            St1 = stokesnr(i)
            St2 = stokesnr(nl)
         else
            gts = stokesnr(nl) / omegaK(g%rce(nr))
            lts = stokesnr(i) / omegaK(g%rce(nr))
            St1 = stokesnr(nl)
            St2 = stokesnr(i)
         endif
      ...
      vtan(:) = vn * ( 1. /(1.+stokesnr(:)**2.) - 1. / (1.+stokesnr(nl)**2.) ) ! azimuthal drift
      relvels(nl,:) = sqrt(vB2(:) + (vs(:) - vs(nl))**2 + vT2(:) + (vr(:) - vr(nl))**2 + vtan(:)**2)
   ...
   subroutine vel_vs_centr(nr, ni, i, stokesnr, vs)
      implicit none
      ...
      vs(i) = g%zce(nr,ni) * omegaK(g%rce(nr)) * min(stokesnr(i), 0.5)
