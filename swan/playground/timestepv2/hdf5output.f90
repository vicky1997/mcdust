...
module hdf5output
  ...
  contains
  subroutine hdf5_file_write (this, swrm, time, mode, outnr,mswarm)
    ...
    if (mode .eq. 'create') then
      call h5open_f(error)
      call hdf5_file_create(this, filepath, outnr , mswarm)     ! creates file
...
! subroutine to add the swarm object as a compound datatype to the HDF5 file
  ...
  subroutine hdf5_file_create(this, filename, nout, mswarm)
    implicit none
    type(hdf5_file_t), intent(out) :: this
  ...
  subroutine hdf5_file_read(Ntot, swrm, nout, mswarm, time)
    implicit none
    ...
    time = time * year
    call hdf5_read_att(nout,mswarm)
    call h5close_f(error)
  ...
  subroutine hdf5_read_att(nout, mswarm)
    implicit none
    type(hdf5_file_t) :: this
    type(C_PTR) :: f_ptr
    real, target :: outnr, mswarm
    integer(HSIZE_T), dimension(1) :: dims
    integer(HID_T) :: att1, att2
    integer :: nout
