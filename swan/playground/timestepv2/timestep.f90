   ...
   subroutine time_step(bin, ncolls, dtout, final_dtime)
      ...
      do i = 1, size(ncolls(:,:),dim=1)
        do j = 1, size(ncolls(:,:),dim=2)
          ...
          if (vertsett) final_dtime = min(final_dtime, c2*g%dz(i,j)/maxval(abs(bin(i,j)%p(:)%velz))) ! don't let particles jump over cells in z
          if (rdrift) final_dtime = min(final_dtime, c2*g%dr(i)/maxval(abs(bin(i,j)%p(:)%velr))) ! don't let particles jump over cells in r
        enddo
      enddo
